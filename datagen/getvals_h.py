...
def bb2array(b, flip=False):
    x = numpy.zeros(65, dtype=numpy.float32)
...
# Generate all boards
    ...
    while gn:
        ...
        command = engine.go(async_callback=True, movetime = 0.1, ponder=False)
        command.result()
...
def samplefunc():
    f = open('/home/jps8/datagen/biggames.pgn')
    b1 = []
    b2 = []
    l1 = []
    l2 = []
    ...
    #print "we have", len(bindivs), "bins divided by"
    #for i in bindivs:
    #    print i, ", "
    #print
    ...
    print "b1 is ", len(l1)
    b1 = numpy.array(b1).reshape(train_boards, 1, 65)
    b2 = numpy.array(b2).reshape(test_boards, 1, 65)

    print "test boards ", test_boards
    l1 = l1.reshape(train_boards,1, 1)
    l2 = l2.reshape(test_boards,1, 1)
    ...
    name1 =str("./b1_" + special + ("t.tensor"))
    t1.name = 'boards train '+ str(data_num)
    t1.value = b1.astype(numpy.float32)
    marvin_io.write_tensor(name1, t1)
    testt(name1)

    t2.name = 'boards test '+ str(data_num)
    t2.value = b2.astype(numpy.float32)  
    name2 =str("./b2_" + (str(data_num)) +special + ("t.tensor"))
    marvin_io.write_tensor(name2, t2)
    testt(name2)

    t3.name = 'labels train '+ str(data_num)
    t3.value = l1.astype(numpy.float32)
    name3 =str("./l1_" +special + ("t.tensor"))
    marvin_io.write_tensor(name3, t3)
    testt(name3)

    t4.name = 'labels test '+ str(data_num)
    t4.value = l2.astype(numpy.float32)
    name4 =str("./l2_" +special +("t.tensor"))
